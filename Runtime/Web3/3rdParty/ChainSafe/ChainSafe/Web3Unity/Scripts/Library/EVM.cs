...
public static async Task<List<MintedNFT.Response>> GetMintedNFT(string _chain, string _network, string _account)
{
    ...
    public static async Task<string> AllErc721(string _chain, string _network, string _account, string _contract = "", int _first = 500, int _skip = 0)
    {
        ...
        Debug.Log("_chain: " + _chain);
        Debug.Log("_network: " + _network);
        Debug.Log("_account: " + _account);
        Debug.Log("_contract: " + _contract);
        Debug.Log("_first: " + _first);
        Debug.Log("_skip: " + _skip);
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Post(url, form))
        {
            ...
            if (response.Equals("please verify that network and chain properly defined in the request", 
                    StringComparison.OrdinalIgnoreCase))
            {
                Debug.LogError("response: " + response);
                return null;
            }
            Response<string> data = JsonUtility.FromJson<Response<string>>(response);
            return data.response;
        }
    }
