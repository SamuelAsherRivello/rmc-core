...
public class Web3Wallet
{
    ...
    public static async Task<string> SendTransaction(string _chainId, string _to, string _value, string _data = "", string _gasLimit = "", string _gasPrice = "")
    {
        ...
        // SRIVELLO - Checks ADDED
        while (clipBoard == "" || !clipBoard.StartsWith("0x") || clipBoard.Length != 66)
        {
            clipBoard = GUIUtility.systemCopyBuffer;
            
            //SRIVELLO - Allow screen to refresh
            await UniTask.Delay(100, DelayType.Realtime, PlayerLoopTiming.Update);
        }
    ...
    public static async Task<string> Sign(string _message)
    {
        ...
        // SRIVELLO - Checks ADDED
        while (clipBoard == "" || !clipBoard.StartsWith("0x") || clipBoard.Length != 132)
        {
            clipBoard = GUIUtility.systemCopyBuffer;
